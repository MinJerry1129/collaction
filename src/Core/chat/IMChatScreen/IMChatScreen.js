...
const IMChatScreen = (props) => {
  ...
  const onAudioRecordSend = (audioRecord) => {
    startUpload({ ...audioRecord, source: audioRecord.uri });
  };
  ...
  const onLaunchCamera = () => {
    ...
    ImagePicker.openCamera({
      ...
      .then((image) => {
        ...
        const data = {
          content: '',
          createdAt: channelManager.currentTimestamp(),
        ...
        if (includesVideo) {
          compressVideo(source, (newSource) => {
            startUpload({ source: newSource, mime });
          });
        } else {
          resizeImage({ uri: source }, (uri) => {
            startUpload({ source: uri, mime });
          });
        }
      })
